...
//添加学生
int MsSqlDbOperate::AddStudent(const wchar_t* sno, const wchar_t * name, const wchar_t* appid
	, const int handup, const wchar_t* stream_ip)
{
	int ret = 0;
	wchar_t strsql[MAX_PATH];
	wsprintfW(strsql, L"INSERT INTO student_info VALUES('%s', '%s', '%s', '%d', '%s');",
		sno, name, appid, handup, stream_ip);
	ret = ExecDirect(strsql);
	if (ret == -1)
		return -1;
	return 0;
...
std::wstring MsSqlDbOperate::QueryStudentIp(wchar_t * sno)
{
	...
	wsprintfW(strsql, L"SELECT * FROM student_info WHERE sno = '%s'", sno);
	mutex_.lock();
	ret = SQLAllocHandle(SQL_HANDLE_STMT, hdbc_, &hstmt_);
	ret = SQLExecDirectW(hstmt_, (SQLWCHAR*)strsql, SQL_NTS);
	...
	while ((ret = SQLFetch(hstmt_)) != SQL_NO_DATA)
	{
		SQLGetData(hstmt_, 5, SQL_C_WCHAR, stream_ip, 128, &Cbappid);
	}
